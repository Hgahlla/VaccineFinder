...
from open_street_maps import OSMClient
...
def populate_location_db(data):
    ...
    if count == 0:
        for loc in data['responsePayloadData']['data']['TX']:
            ...
            lat, long = OSMClient.get_lat_long(loc['city'] + ', ' + loc['state'])
...
def get_locations_within_travel_time(source, locations, hr, m):
    ...
    lat, long = OSMClient.get_lat_long(source)
    for loc in locations:
        maps = OSMClient(lat, long, loc.latitude, loc.longitude)
        maps.get_route()
