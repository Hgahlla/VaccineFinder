...
class Location:
    ...
    def save_to_db(cls, city, state, lat, long, status):
        with CursorFromConnectionPool() as cursor:
            cursor.execute("INSERT INTO location (city, state, latitude, longitude, status) VALUES (%s, %s, %s, %s, %s)",
                           (city, state, lat, long, status))

    @classmethod
    def update_status(cls, city, status):
        with CursorFromConnectionPool() as cursor:
            cursor.execute("UPDATE location SET status = (%s) WHERE city = (%s) AND status <> (%s)", (status, city, status))
    ...
    def from_json(cls, data):
        ...
        for loc in data['responsePayloadData']['data']['TX']:
            loc_list.append(Location(loc['city'], loc['state'], loc['status']))
